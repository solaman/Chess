...
public class ChessBoard {
	 ...
	 */
	protected List<ChessMove> movesToClear;
	
	/**
	 * used to locally store moves that need to build their data onto the board.
	 */
	protected List<ChessMove> movesToBuild;
	...
	public ChessBoard( int xLength, int yLength ) throws Exception{
		...
		movesToClear= new ArrayList<ChessMove>();
		movesToBuild= new ArrayList<ChessMove>();
	...
	private final boolean hasLegalMove(ChessPiece piece) throws Exception{
		List<CommandSequence> commandSequences= piece.getAllCommandSequences(this);
		for(CommandSequence commandSequence : commandSequences){
	 ...
	 */
	protected boolean isLegalMove(CommandSequence commandSequence) throws Exception{
	...
	public boolean canThreaten(ChessPiece piece, ChessSpace spaceToCheck){
		List<ChessMove> moveRefs= spaceToCheck.getDependentMoves();
		for(ChessMove move : moveRefs)
	...
	public boolean performMovePermanent(JSONObject commandSequence) throws ClassNotFoundException, JSONException, Exception{
		return performMovePermanent( new CommandSequence(this, commandSequence));
	}
	 ...
	 * @throws Exception -player of piece performing move does not have a king
	 */
	public boolean performMovePermanent( CommandSequence commandSequence) throws Exception{
		if(!isLegalMove(commandSequence))
			return false;
		commandHistory.doMove(commandSequence, this);
		updateAllMoves();
	...
	private final void updateAllMoves() {
		for(ChessMove move : movesToClear)
			clearMoveData(move);
	...
	private void addToList(List<ChessMove> toAdd, List<ChessMove> moveList){
		for(ChessMove move: toAdd)
